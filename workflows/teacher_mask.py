...
class MaskTeacher:
    ...
    def _run_semantic_segmentation(self):
        if self.model_name == "sam2":
            ...
            for sam_model in sam_models:
                ...
                if prompt_field:
                    label_field_bbox_prompt = (
                        f"pred_{model_name_clear}_prompt_field_{prompt_field}"
                    )
                else:
                    label_field_bbox_prompt = f"pred_{model_name_clear}_no_prompt_field"

                self.inference_sam2(
                    self.dataset, sam_model, label_field_bbox_prompt, prompt_field
                )

        logging.info("Semantic segmentation completed for all models.")

    def inference_depth_estimation(
        self, dataset, model_name, label_field, prompt_field=None
    ):
    ...
    def _run_depth_estimation(self):
        if self.model_name in ["dpt", "depth_anything", "glpn", "zoe_depth"]:
            ...
            prompt_field = self.model_config.get(
                "prompt_field", None
            ...
            for depth_model in depth_models:
                ...
                label_field_with_prompt = (
                    f"pred_{model_name_clear}_prompt_{prompt_field}"
                    if prompt_field
                    else None
                )

                if prompt_field:
                    self.inference_depth_estimation(
                        self.dataset, depth_model, label_field_with_prompt, prompt_field
                    )
                else:
                    self.inference_depth_estimation(
                        self.dataset, depth_model, label_field_no_prompt
                    )
