...
class AwsDownloader:
        ...
        sample_fields = [
            {
                ...
                },  # FIXME Replace with actual date of function call
            },
            {
                "name": "sensor",
                "ftype": "fiftyone.core.fields.StringField",
            ...
            for line in file:
                try:
                    ...
                    if "time" in data and "data" in data:
                        ...
                        formatted_time = (
                            time_obj.strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3] + "Z"
                        )
                        ...
                        formatted_time = (
                            michigan_time.strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3] + "Z"
                        )
                    else:
                    ...
                    if image_base64 and formatted_time:
                        ...
                        if (
                            sensor_name is None
                        ):  # FIXME Can be derived from S3 source if not stored in data itself
                            sensor_name = "Unknown"
                        ...
                        formatted_time = (
                            formatted_time.split(".")[0] + "." + milliseconds + "Z"
                        )
                        ...
                        # Store image to disk
                        output_path = os.path.join(output_folder_data, image_filename)
                        ...
                        logging.error(
                            f"There was an issue during file processing of {file_path}. Issues with image_base64: {image_base64 is None}, formatted_time: {formatted_time is None}, sensor_name: {sensor_name is None}"
                        )
                        continue

                except json.JSONDecodeError as e:
                    logging.error(
                        f"File {os.path.basename(file_path)} - Error decoding JSON: {e}"
                    )
    ...
    def download_files(self, log_dir, MAX_SIZE_TB=1.5):
        ...
        if total_size_tb <= MAX_SIZE_TB:
            ...
            logging.error(
                f"Total size of {total_size_tb:.2f} TB exceeds limit of {MAX_SIZE_TB} TB. Skipping download."
            )

        logging.info(
            f"Collected {n_downloaded_files} files of total size {total_size_tb:.2f} TB, skipped {n_skipped_files} files."
        )
        ...
        subfolders = [
            f.path for f in os.scandir(self.download_path) if f.is_dir()
        ]  # Each subfolder stands for a requested sample rate. Defaults to "1"
        if len(subfolders) > 0:
            sub_folder = os.path.basename(subfolders[0])
            if len(subfolders) > 1:
                logging.warning(
                    f"More than one subfolder in download directory {self.download_path} found, selected {sub_folder}."
                )
            ...
            with tqdm(
                total=len(files), desc="Checking downloaded files", unit="file"
            ) as pbar:
                for file in files:
            ...
            logging.info(
                f"Downloaded {len(downloaded_files)} files. Total size: {downloaded_size_tb:.2f} TB"
            )
            ...
            if len(files_to_be_downloaded) == len(downloaded_files):
                ...
                logging.error(
                    f"Only {len(downloaded_files)} of {len(files_to_be_downloaded)} planned files downloaded."
                )
            ...
            if total_size_tb == downloaded_size_tb:
                ...
                logging.error(
                    f"The downloaded size of {downloaded_size_tb} TB varies from the planned download size of {total_size_tb} TB."
                )
                DOWNLOAD_SIZE_SUCCESS = False
        else:
            sub_folder = None
            logging.error(
                f"No subfolder found in download directory {self.download_path}."
            )
    ...
    def decode_data(
        ...
        logging.info(
            f"Added {len(files)} files to task queue. Ready for multiprocessing."
        )
        ...
        result_worker_process = mp.Process(
            target=self.result_json_worker,
            args=(result_queue, json_file_path, worker_done_events, log_dir),
        )
        ...
        for done_event in worker_done_events:
            p = mp.Process(
                target=self.data_extraction_worker,
                args=(
                    task_queue,
                    result_queue,
                    done_event,
            ...
            if task_queue.empty() == True:  # If queue is empty, break out
                ...
                try:
                    file_path = task_queue.get(
                        timeout=0.1
                    )  # Get a file path from the task queue
                    ...
                    if len(v51_samples) > 0:
                        ...
                        if n_files_processed % 100 == 0:
                            logging.info(
                                f"Worker {os.getpid()} finished {n_samples_processed} samples. {n_files_processed} / ~{n_files_per_worker} files done."
                            )
                        n_files_processed += 1
                    ...
                    logging.error(
                        f"Error occured during processing of file {os.path.basename(file_path)}: {e}"
                    )
                    continue
    ...
    def result_json_worker(
        ...
        while len(worker_done_events) > 0 or not result_queue.empty():
            try:
                ...
                writer.add_scalar(
                    "decode/samples", n_images_extracted, n_files_processed
                )
                n_files_processed += 1
                ...
                if n_files_processed % 1_000 == 0:
                    logging.info(
                        f"{n_images_extracted} samples added to dict. Items in queue: {result_queue.qsize()}. Active workers: {len(worker_done_events)}"
                    )
